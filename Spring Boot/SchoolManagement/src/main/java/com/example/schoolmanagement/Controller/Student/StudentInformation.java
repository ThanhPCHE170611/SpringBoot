...
public class StudentInformation {
    ...
    public String updateStudentprofile(@RequestParam(name = "picture", required = false)MultipartFile picture, @RequestParam("fullname") String fullname,
        ...
        if(!isFullNameValid(fullname)){
            ...
            return getStudentInformation(session, model);
        }
        ...
        if(canUpdate && picture.isEmpty()){
            studentInformationService.updateStudentWithoutPicture(session,student ,fullname, newpassword, gender, ethnic, religion, parrentname, address, hometown, hobbies);
            model.addAttribute("error", "Update information successfully!");
            return getStudentInformation(session, model);
        }// update avatar
        else if(canUpdate && !picture.isEmpty()){
            studentInformationService.updateStudentWithPicture(picture, session,student ,fullname, newpassword, gender, ethnic, religion, parrentname, address, hometown, hobbies);
            model.addAttribute("error", "Update information successfully!");
            return getStudentInformation(session, model);
        }
        return getStudentInformation(session, model);
